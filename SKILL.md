...
"Looks finished" is not finished. On 2026-09-13 the founder audited Hermes Startup's live site against a public twenty-item launch checklist. Two gaps surfaced (no `favicon.ico`, no home-screen icon) and both were fixed the same day. Walk the same ground before you call a page done.

1. Every page gets a title and a description. The title names the outcome; the description says what the reader gets. Search results and link previews show both.
2. One action above the fold. Without scrolling, a visitor sees the single thing to do: copy the prompt, answer the free questions, buy. On a long page, keep that action reachable.
3. Serve your own 404. A wrong URL shows your page with your navigation, never the browser's dead end.
4. The full icon set. `favicon.ico` (browsers still request it by name), a 512px icon in the web manifest, an `apple-touch-icon` for home screens, and one Open Graph image so shared links preview properly.
5. `robots.txt` and `sitemap.xml` exist. Allow the crawlers you want by name and list your pages in the sitemap. "Check if AI assistants can find your site" covers the crawler detail.
6. Alt text on every image, and compressed images. Alt text is what screen readers and AI crawlers read; compression is what a phone on a slow connection can load.
7. Test on a real phone, not a resized window. Check the breakpoints, then open the live site on an actual device. Most visitors arrive there.
8. A privacy page and a terms page in plain language: what you collect, what you charge, what you do not.
9. No cookies means no cookie banner. A banner over a site that sets no cookies costs trust and buys nothing. Hermes Startup counts visits anonymously, first-party, with no cookies.
10. A form survives a failed send. Keep what the visitor typed when the network drops, and say plainly it did not send.

Fast path: fetch each live URL fresh, walk the list, fix what is missing. The check takes minutes; skipping it is what ships a broken share preview or a dead end where a page should be.

Honest framing: this list makes a site real and safe to visit. It does not make it sell; the offer and the copy still do that. Adapted from a public checklist by @Manixh02 (2026-09-13).
